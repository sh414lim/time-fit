...
export async function runCardSync({ connection, runId, mode = 'incremental', userToken = null }) {
  ...
  await financeRest(`timefit_user_card_sync_runs?id=eq.${encodeURIComponent(runId)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ status: 'running', heartbeat_at: startedAt, lease_expires_at: new Date(Date.now() + 330000).toISOString(), started_at: startedAt }) });
  try {
    ...
    let imported = 0; let duplicates = 0; let groups = 0; let requestCount = 0;
    for (const card of cards) {
      ...
      requestCount += 1;
      const acquisitions = provider.fetchAcquisitions ? await provider.fetchAcquisitions({ organizationId: connection.organization_id, card, mode, ...window }) : [];
      if (provider.fetchAcquisitions) requestCount += 1;
      const result = await importEvents(userToken, connection.organization_id, card.id, connection.provider, [...events, ...acquisitions]);
    ...
    await financeRest(`timefit_user_card_sync_runs?id=eq.${encodeURIComponent(runId)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ status: 'succeeded', request_count: requestCount, inserted_count: imported, duplicate_count: duplicates, lease_owner: null, lease_expires_at: null, heartbeat_at: finishedAt, finished_at: finishedAt }) });
    ...
    await financeRest(`timefit_user_card_sync_runs?id=eq.${encodeURIComponent(runId)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ status: 'failed', failed_count: 1, error_code: code, error_category: category, lease_owner: null, lease_expires_at: null, heartbeat_at: finishedAt, finished_at: finishedAt }) }).catch(() => {});
