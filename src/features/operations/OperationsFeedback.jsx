...
export function WeeklyFeedback({ organizationId, accountId, detailed = false, initialFrom, onNavigate, refreshToken }) {
  ...
  return <section className="card ops-card ops-weekly">
    ...
    {data && (!data.connected ? <p className="ops-status">연결된 매출 데이터가 없습니다. 운영 설정에서 POS 연결을 확인해 주세요.</p> : <>
      ...
      <div className="ops-metrics">{[[data.comparable ? '완료 매출' : '수록된 완료 매출', money(data.current.revenue), data.current.revenue, data.previous.revenue], [data.comparable ? '완료 주문' : '수록된 완료 주문', `${number(data.current.orders)}건`, data.current.orders, data.previous.orders], ['주문당 금액', data.current.average === null ? '—' : money(data.current.average), data.current.average, data.previous.average]].map(([label, value, current, previous]) => <article key={label}><span>{label}</span><strong>{value}</strong><small>{current === null ? '완료 주문 없음' : delta(current, previous)}</small></article>)}</div>
      ...
      {detailed && tab === 'sales' && <div className="ops-days"><p className="ops-note">진한색 이번 주 · 옅은색 비교 주{!data.comparable ? ' (비교 보류)' : ''}</p>{data.current.days.map((day, index) => {
        ...
        return <div className="ops-day" key={day.date}><b>{['월','화','수','목','금','토','일'][index]}<small>{day.date.slice(5)}</small></b><div><div className={`ops-bar ${day.covered ? '' : 'unverified'}`}><i style={{ width: `${day.revenue / max * 100}%` }}/><span>{day.covered ? money(day.revenue) : day.revenue > 0 ? `저장된 주문 ${money(day.revenue)} · 수집 미확정` : '자료 미확인 · 매출 미확정'}</span></div>{data.comparable && <div className="ops-bar previous"><i style={{ width: `${data.previous.days[index].revenue / max * 100}%` }}/><span>{money(data.previous.days[index].revenue)}</span></div>}</div></div>;
      })}</div>}
