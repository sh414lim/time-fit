...
function DailyAnalysis({ data }) {
  ...
  return <section className="sales-daily-section"><div className="sales-section-head"><div><span>일별 흐름</span><h3>어느 요일에서 차이가 생겼는지 확인하세요</h3><p>막대를 선택하면 매출·주문·객단가·취소를 함께 볼 수 있습니다.</p></div><div className="sales-metric-switch" aria-label="그래프 지표">{[['revenue','매출'],['orders','주문 수'],['average','주문당 금액']].map(([key, text]) => <button key={key} aria-pressed={metric === key} onClick={() => setMetric(key)}>{text}</button>)}</div></div><div className="sales-chart-legend"><span className="current">선택 주</span><span className="previous">비교 주</span><em>그래프 최대값 {label(max)}</em></div><div className="sales-comparison-chart" role="img" aria-label="요일별 선택 주와 비교 주 그래프">{data.current.days.map((day, index) => { const previous = data.previous.days[index]; const trend = comparison(value(day), value(previous), metric === 'orders' ? '건' : '원', data.comparable); return <button type="button" className={selected === index ? 'selected' : ''} onClick={() => setSelected(index)} key={day.date}><span className="sales-chart-values"><b>{label(value(day))}</b>{data.comparable && <small>{trend.amount >= 0 ? '+' : '−'}{number(Math.abs(trend.amount))}{metric === 'orders' ? '건' : '원'}</small>}</span><span className="sales-chart-bars"><i className="current" style={{ height: `${Math.max(3, value(day) / max * 100)}%` }}/>{data.comparable && <i className="previous" style={{ height: `${Math.max(3, value(previous) / max * 100)}%` }}/>}</span><strong>{weekday[index]}<small>{day.date.slice(5)}</small></strong></button>; })}</div><DayDetail day={data.current.days[selected]} previous={data.previous.days[selected]} index={selected}/><div className="sales-day-table"><div className="sales-day-table-head"><span>요일</span><span>완료 매출</span><span>전주 대비</span><span>주문</span><span>객단가</span><span>취소</span></div>{data.current.days.map((day, index) => { const previous = data.previous.days[index]; const trend = comparison(day.revenue, previous.revenue, '원', data.comparable); return <button key={day.date} onClick={() => setSelected(index)}><span><b>{weekday[index]}요일</b><small>{day.date}</small></span><strong>{money(day.revenue)}</strong><Trend value={trend}/><span>{number(day.orders)}건</span><span>{day.average === null ? '—' : money(day.average)}</span><span>{number(day.cancelled)}건</span></button>; })}</div></section>;
}
