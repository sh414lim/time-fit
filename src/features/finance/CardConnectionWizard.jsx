...
export default function CardConnectionWizard() {
  ...
  useEffect(() => {
    ...
    getAuthContext().then(async context => {
      ...
      if (current) { setConnection(current); setStep(['active', 'degraded', 'reauth_required'].includes(current.status) ? 'complete' : 'cards'); }
  ...
  return <div className="connection-wizard">
    ...
    {step === 'complete' && <div className="connection-complete"><div><b>{connection?.status === 'reauth_required' ? '카드사 재인증이 필요해요' : connection?.status === 'degraded' ? '자동 수집을 다시 확인해 주세요' : '카드 자동 연결이 완료됐어요'}</b><span>{connection?.status === 'reauth_required' ? '카드사 인증이 만료되었거나 변경되었습니다. 인증정보 갱신 전까지 자동 재시도하지 않습니다.' : connection?.status === 'degraded' ? `${connection.last_error_category || '일시 오류'} · ${connection.next_sync_at ? `${new Date(connection.next_sync_at).toLocaleString('ko-KR')} 자동 재시도` : '재시도 대기 중'}` : result ? `신규 ${result.imported || 0}건 · 중복 제외 ${result.duplicates || 0}건 · 거래 ${result.groups || 0}개` : '마지막으로 연결된 카드의 자동 수집 상태입니다.'}</span></div>{connection?.status === 'degraded' ? <button className="outline" disabled={busy} onClick={retrySync}>{busy ? '동기화 중…' : '지금 다시 동기화'}</button> : connection?.status === 'reauth_required' ? <button className="outline" onClick={() => setError('카드사 재인증 화면은 다음 연결 단계에서 제공됩니다.')}>재인증 안내</button> : <button className="outline" onClick={() => { setStep('cards'); setError(''); loadAssets(); }}>카드 다시 선택</button>}</div>}
