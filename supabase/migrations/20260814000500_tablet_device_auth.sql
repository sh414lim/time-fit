-- Replaces shared tablet PIN authentication with revocable business-scoped device tokens.
create table if not exists public.timefit_user_tablet_devices (
...
create index if not exists timefit_user_tablet_devices_organization_idx on public.timefit_user_tablet_devices(organization_id, status);
create trigger timefit_user_tablet_devices_updated_at before update on public.timefit_user_tablet_devices for each row execute procedure public.set_updated_at();

alter table public.timefit_user_tablet_devices enable row level security;
create policy "manager manages tablet devices" on public.timefit_user_tablet_devices for all using (public.timefit_user_has_membership_role(organization_id, array['manager']::public.timefit_user_role[]));
grant select, update on public.timefit_user_tablet_devices to authenticated;
...
begin
  ...
  values(p_organization_id, trim(p_display_name), encode(digest(v_token, 'sha256'), 'hex'), auth.uid(), coalesce(p_metadata, '{}'::jsonb)) returning * into v_device;
...
begin
  select * into v_device from public.timefit_user_tablet_devices where token_hash=encode(digest(p_device_token, 'sha256'), 'hex') and status='active' and expires_at>now();
...
grant execute on function public.timefit_user_activate_tablet_device(uuid,text,jsonb) to authenticated;
grant execute on function public.timefit_user_tablet_device_context(text), public.timefit_user_tablet_attendance_v2(text,text,text), public.timefit_user_tablet_leave_request_v2(text,text,date,date,text) to anon, authenticated;
