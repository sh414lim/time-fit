-- Secure, idempotent event import used by CSV today and provider adapters later.
...
begin
  ...
  where id = p_corporate_card_id and organization_id = p_organization_id and archived_at is null
  for update;
  ...
  loop
    ...
    insert into public.timefit_user_card_transaction_groups(
      ...
      p_organization_id, p_corporate_card_id, v_group_key,
    ...
    do update set
      ...
      merchant_name = coalesce(excluded.merchant_name, public.timefit_user_card_transaction_groups.merchant_name),
      updated_at = now()
    ...
    insert into public.timefit_user_card_transaction_events(
      ...
      provider_event_id, event_type, occurred_at, amount, currency,
      approval_number, original_provider_event_id, merchant_name,
      merchant_business_number, raw_checksum, imported_by
    ) values (
      p_organization_id, v_group_id, p_corporate_card_id, p_provider,
      v_provider_event_id, v_event->>'eventType', (v_event->>'occurredAt')::timestamptz,
      (v_event->>'amount')::bigint, coalesce(nullif(v_event->>'currency',''),'KRW'),
      nullif(v_event->>'approvalNumber',''), nullif(v_event->>'originalProviderEventId',''),
      nullif(v_event->>'merchantName',''), nullif(v_event->>'merchantBusinessNumber',''),
      nullif(v_event->>'rawChecksum',''), auth.uid()
  ...
  foreach v_group_id in array v_affected_groups
  loop
    select
      coalesce(sum(amount) filter (where event_type = 'approval'),0),
    ...
    v_status := case
      when v_cancelled > 0 and v_net = 0 then 'cancelled'
      when v_cancelled > 0 then 'partially_cancelled'
      when v_billed is not null then 'billed'
      when v_acquired > 0 then 'acquired'
      else 'pending'
    end;

    update public.timefit_user_card_transaction_groups
    set approved_amount = v_approved,
        acquired_amount = v_acquired,
        cancelled_amount = v_cancelled,
        net_amount = v_net,
        status = v_status,
...
grant execute on function public.timefit_user_import_card_events(uuid,uuid,text,jsonb) to authenticated;
