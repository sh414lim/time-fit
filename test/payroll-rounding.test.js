...
test('버터빌라 10분 올림: 20분은 20분, 21분은 30분 급여시간이다', () => {
  const policy = { payroll_rounding_enabled: true, attendance_rounding_minutes: 10, attendance_rounding_mode: 'ceil' };
  assert.equal(roundPayableMinutes(1, policy), 10);
  assert.equal(roundPayableMinutes(20, policy), 20);
  assert.equal(roundPayableMinutes(21, policy), 30);
